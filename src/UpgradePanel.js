...
const UpgradePanel = (props) => (
    <div className="row">
        ...
        <div className="col-12">
            <div className="row">
                <h3 className="col-12">Specials</h3>
                {props.specials.map((special) => (
                    <div className="col-md-3 p-1">
                        <div key={special.id} className="card">
                            <div className="card-body">
                                <h5 className="card-title">{special.name}</h5>
                                <h6 className="card-subtitle mb-2 text-muted">Special Ability: {special.strength}</h6>
                                {/* <p className="card-text"></p> */}
                                <button className="btn btn-primary"
                                    type="special"
                                    id={special.id}
                                    onClick={props.upgradeHandler}
                                    disabled={props.loot < special.cost}>
                                    Buy for {special.cost} loot
                                </button>
                            </div>
                        </div>
                    </div>
